...
<title>BookBid: Sorted Auction List - Low to High</title>
</head>
</head>
	<style>
		body {
...
<br>
		...
		try {
			...
			PreparedStatement prstmt = con.prepareStatement("select name, item_id from items where bought is not true order by current_price asc");
			...
			if (!result.next()) {
			    ...
			    while(result.next()) {
			        ...
			        out.println("<td><a style='font-size:18px' href='BuyPage.jsp?num=" + i + "'>" + itemName + "</a></td>");

			        request.getSession().setAttribute("selectedItemID" + i, itemId);
			        request.getSession().setAttribute("selectedItemName" + i, itemName);
